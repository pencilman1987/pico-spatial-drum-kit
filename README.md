...
一个面向 PICO Spatial SDK 的原生空间架子鼓应用。默认开启无手柄的双手追踪模式：用户通过捻合或握拳模拟握槌，手势敲击点向下穿越鼓面时，应用根据速度触发对应鼓采样。头显内可完成手势、速度、整体鼓组位置、逐鼓面位置和软件延迟校准。

## 主要功能

- PICO Mixed Stage 中的人体尺度八件套架子鼓
- 启动时将用户水平原点对齐真实三脚鼓凳中心，并让两个吊嗵鼓的中线正对用户前方
- 8 个命中面均从正式 GLB 网格实测，包含斜面法线、可敲击半径和统一模型变换
- 无需手柄的双手追踪，支持捻合/握拳识别和扫掠式鼓面穿越检测
- HUD 显示左右手识别状态，并可随时暂停或恢复手势敲鼓
- 不显示鼓槌的手势敲击延伸、力度映射及 140ms 命中反馈
- 8 个本地 CC0 WAV 鼓采样，启动时校验授权记录与 SHA-256
- Snare、Kick 和整套鼓的模拟器试听入口
- 头显内手势阈值、敲击速度、鼓组整体位置和逐鼓面位置校准
...
鼓组按坐姿高度放置，默认水平视角下可能位于画面下方。在 PICO Emulator 中按住鼠标右键拖动视角，点击 HUD 的“调整鼓组位置”可实时微调左右、上下和远近；点击“试听军鼓”可验证声音。

模拟器可以验证模型加载、采样加载、模式开关和按钮播放，但不会生成真实的 26 关节手部追踪数据，因此不能替代真机敲击、手势阈值、鼓面位置及声学端到端延迟校准。完整步骤见 [CALIBRATION.md](CALIBRATION.md)。
...
- 22 个 JVM 单元测试通过，包含鼓凳原点、演奏朝向、PICO 旋转约定、实测鼓面、捻合、握拳、张手和手势模式开关
- 3 个 PICO Emulator 设备端测试通过，包含 8 个授权鼓采样的加载与播放提交验证
