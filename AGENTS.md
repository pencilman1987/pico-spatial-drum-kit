...
- `interaction/HandGestureRecognizer.kt` is the pure, unit-tested pinch/fist classifier. Hand gesture mode is enabled by default and requires no controller.
...
- `content/DrumScene.kt` owns ECS entities and resources. It also converts Tracking Pack world-space joint positions into the active SpatialView's local meter space before hit testing.
...
- `DrumKitModelPlacement` records the measured black three-legged throne anchor and rack-tom playing center. The production GLB is yawed 201.79572° so the rack-tom centerline faces Stage -Z, and the real throne center starts under the viewer origin. The old anchor was the floor-tom head and must not be restored.
- `domain/model/DrumKit.kt` stores eight mesh-measured source-space playing centers, normals, and radii, then applies the exact production-model transform. Do not replace them with visually estimated Stage coordinates.
- `calibration/CalibrationProfile.kt` and `CalibrationStore.kt` own persistent device-specific thresholds and per-drum offsets.
- `CalibrationStore` uses layout schema 2; older saved kit/surface offsets are ignored because they were relative to the incorrect floor-tom anchor, while gesture and latency values remain reusable.
...
- Runtime left/right hand-presence feedback and an explicit pause/resume control for gesture drumming
...
- The model orientation evidence is stored in `.spatialsdk/asset_orientations.json`; the source forward axis runs from the real throne to the rack-tom midpoint.
